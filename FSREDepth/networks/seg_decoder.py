...
class SegDecoder(nn.Module):
    ...
    def forward(self, input_features):
        ...
        bottom_label = outputs[('seg_logits', max(self.scales))]
        # bottom_label = torch.
        bottom_label = torch.argmax(bottom_label, dim=1)  # Convert logits to class labels
        bottom_label = F.interpolate(bottom_label.unsqueeze(1).float(), 
                                    size=(input_features[0].shape[2], input_features[0].shape[3]), 
                                    mode='nearest').squeeze(1).long()
        return outputs, bottom_label
