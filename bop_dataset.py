...
class bop_dataset_single_obj_pytorch(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:           
            ...
            roi_x = get_roi(x, Bbox, self.crop_size_img, interpolation=cv2.INTER_LINEAR, resize_method="crop_square_resize")
...
if __name__ == '__main__':
    ...
    import pprint
    a = bop_io.get_dataset('lm/','lmo', train=True, data_folder='lm/lmo/train', data_per_obj=True, incl_param=True, train_obj_visible_theshold=0.2)
    # pprint.pprint(a)
    from tools_for_BOP.common_dataset_info import get_obj_info
    
    # dataset_dir,source_dir,model_plys,model_info,model_ids,rgb_files,depth_files,mask_files,mask_visib_files,gts,gt_infos,cam_param_global, cam_params = bop_io.get_dataset('lm/','lmo', train=True, data_folder='lm/lmo/train_pbr', data_per_obj=True, incl_param=True, train_obj_visible_theshold=0.2)
    dataset_dir,source_dir,model_plys,model_info,model_ids,rgb_files,depth_files,mask_files,mask_visib_files,gts,gt_infos,cam_param_global, cam_params = bop_io.get_dataset('lm/','lmo', train=True, data_folder='train_pbr', data_per_obj=True, incl_param=True, train_obj_visible_theshold=0.2)

    print("dataset_dir:", dataset_dir)
    source_dir = 'lm/lmo/train_pbr'
    print("source_dir:", source_dir)

    obj_name_obj_id, symmetry_obj = get_obj_info('lmo')
    obj_name = 'ape'
    print(obj_name_obj_id)
    obj_id = int(obj_name_obj_id[obj_name])  
    print(obj_id)
    ...
    resize_method = "crop_square_resize"  # instead of passing the function itself
    ...
    train_dataset = bop_dataset_single_obj_pytorch(
                                                    dataset_dir,'lm/lmo/train_pbr' , rgb_files[obj_id], mask_files[obj_id], mask_visib_files[obj_id], 
